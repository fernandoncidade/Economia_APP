...
class SobreDialog(QDialog):
    def __init__(self, parent, titulo, texto_fixo, texto_history, detalhes, licencas, sites_licencas, 
                 show_history_text, hide_history_text, show_details_text, hide_details_text, show_licenses_text, hide_licenses_text, 
                 ok_text, site_oficial_text, avisos=None, show_notices_text=None, hide_notices_text=None, Privacy_Policy=None, 
                 show_privacy_policy_text=None, hide_privacy_policy_text=None, info_not_available_text="Information not available", 
        ...
        try:
            ...
            self._tab_show_labels = [
                show_history_text or "",
                show_details_text or "",
                show_licenses_text or "",
                show_notices_text or "",
                show_privacy_policy_text or "",
                show_release_notes_text or "Release Notes"
            ]
            
            self._tab_hide_labels = [
                hide_history_text or (show_history_text or ""),
                hide_details_text or (show_details_text or ""),
                hide_licenses_text or (show_licenses_text or ""),
                hide_notices_text or (show_notices_text or ""),
                hide_privacy_policy_text or (show_privacy_policy_text or ""),
                hide_release_notes_text or (show_release_notes_text or "Release Notes")
            ]
            
            def _update_tab_labels(current_index: int):
                for i, (s, h) in enumerate(zip(self._tab_show_labels, self._tab_hide_labels)):
                    label = h if i == current_index and h else s
                    self.tabs.setTabText(i, label or "")

            self.tabs.currentChanged.connect(_update_tab_labels)
            _update_tab_labels(self.tabs.currentIndex())
