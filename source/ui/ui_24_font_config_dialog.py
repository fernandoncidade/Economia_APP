from PySide6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel, QSpinBox, QCheckBox, QPushButton, QFontComboBox, QGroupBox, QFormLayout, QTableWidget, QWidget)
...
class FontConfigDialog(QDialog):
    ...
    def _refresh_all_result_widgets(self, parent):
        try:
            # Obter configuração atual de fonte
            config = FontManager.get_config()
            ...
            # Nomes esperados (mantido por compatibilidade) e detectar atributos dinâmicos
            result_widgets = {
            ...
            # incluir quaisquer atributos do parent que terminem com esses sufixos
            for name in dir(parent):
                if name.endswith("_result") or name.endswith("_output_table"):
                    result_widgets.add(name)
            ...
            for attr_name in result_widgets:
                ...
                try:
                    # Se o widget implementar API customizada, tentar usar
                    if hasattr(widget, 'refresh_all_fonts'):
                        # tentar chamar passando o font; se falhar, chamar sem argumentos
                        try:
                            widget.refresh_all_fonts(font)

                        except TypeError:
                            widget.refresh_all_fonts()

                        logger.info(f"refresh_all_fonts chamado em: {attr_name}")
                        continue

                    # Widgets Qt padrão: setFont se suportado
                    if hasattr(widget, 'setFont'):
                        widget.setFont(font)
                        widget.update()
                        logger.info(f"Fonte aplicada diretamente em: {attr_name}")

                    # Se for QTableWidget, aplicar fonte também aos itens e cabeçalho
                    ...
                    logger.error(f"Erro ao atualizar fonte no widget {attr_name}: {e_widget}", exc_info=True)
