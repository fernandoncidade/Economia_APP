...
def create_menu_bar(self):
    ...
    try:
        ...
        def export_all(suggested_name="todos_calculos.pdf", *args):
            sections = []
            ...
            add_section(tr("App", "VPL com Impostos"), getattr(self, "vpl_tax_result", None))
            add_section(tr("App", "CAUE - Vida Econômica"), getattr(self, "caue_result", None))
            ...
            self.export_to_pdf(combined, suggested_name)
        ...
        def export_current(*args):
            ...
            mapping = {
                ...
                tr("App", "VPL com Impostos"): (getattr(self, "vpl_tax_result", None), "vpl_impostos.pdf"),
                tr("App", "CAUE - Vida Econômica"): (getattr(self, "caue_result", None), "caue.pdf"),
            }

            # Normalizar para busca mais tolerante
            def _normalize(s):
                if not s:
                    return ""
            ...
            tab_norm = _normalize(tab_name)
            widget = None
            name = None

            # procurar correspondência por normalização
            for key, (w, n) in mapping.items():
                if _normalize(key) == tab_norm:
                    widget, name = w, n
                    break
            ...
            # fallback: chamar export_all mas sugerir um nome baseado na aba ativa
            safe_name = re.sub(r"[^0-9a-z]+", "_", tab_name.lower()).strip("_")
            if not safe_name:
                safe_name = "todos_calculos"

            suggested_filename = f"{safe_name}.pdf"
            export_all(suggested_filename)
