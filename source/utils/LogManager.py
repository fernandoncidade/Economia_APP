...
class LogManager:
    ...
    def _configure_logging(cls):
        try:
            log_dir = os.path.join(os.environ.get('LOCALAPPDATA', os.path.expanduser('~')), 'Economia_APP', 'logs')
            os.makedirs(log_dir, exist_ok=True)
            cls._log_file = os.path.join(log_dir, f'file_economia_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
            ...
            cls._logger = logging.getLogger('FileEconomia')

        except Exception as e:
            user_data_dir = os.path.join(os.path.expanduser('~'), 'AppData', 'Local', 'Economia_APP', 'logs')
            os.makedirs(user_data_dir, exist_ok=True)
            cls._log_file = os.path.join(user_data_dir, f'file_economia_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
